...
class Encoder(nn.Module):
    ...
    def forward(self, x, tau):
        ...
        logits = self.fc(x)  # [B, max_seq_len * vocab_size]
...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        return self.mlp(z_flat)        # [B, output_dim]
...
decoder = Decoder(vocab_size, max_seq_len, hidden_dim, input_dim).to(device)
...
for step in range(max_steps):
    ...
    x = torch.FloatTensor(np.asarray(x)).to(device)  # [B, 1, 28, 28]
